...
const ManageProperties = ( function () {
	...
	function disableMultipleFields( propertyType, inputLabel ) {
		...
		if ( jQuery.inArray( propertyType, singleValues ) ) {
			return true;
		}
		...
		return (
			jQuery.inArray( inputName, multiselectInputs ) ||
				jQuery.inArray( inputName, optionsInputs )
		);
	}
...
*/
		...
		if ( !jQuery.inArray( dataType, filter ) ) {
	...
	function getAvailablePropertiesOfDataType( dataType ) {
		...
		switch ( dataType ) {
			...
			case '_boo':
				var filter = [ '_PVAP' ];
				ret = ret.filter( ( x ) => !jQuery.inArray( x, filter ) );
				break;
	...
	function propertyAllowsMultipleValues( property ) {
		return !jQuery.inArray( property, [
			'label', // custom
			'_TYPE', // type
			'_IMPO', // import
			'_PVAP', // Allows pattern
		...
		switch ( action ) {
			case 'save':
				...
				if (
					...
					// eslint-disable-next-line no-underscore-dangle
					!jQuery.inArray( obj.__pageproperties_preferred_input, optionsInputs )
				) {
					OO.ui.alert(
